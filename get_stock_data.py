#!/usr/bin/env python3
"""
沪深300数据提取器 - Streamlit可视化网页版 V2.0
- 登录baostock获取沪深300成分股
- 获取历史量价数据 + PE/PB/PS/PCF估值指标
- 导出标准格式CSV供预测分析使用
- 工作流: 提取数据 → 导出CSV → 上传预测页面
"""
...
st.set_page_config(page_title="沪深300数据提取器", page_icon="📥", layout="wide")
...
st.markdown("<p style='text-align:center; color:#64748b;'>提取数据 → 导出CSV → 上传预测页面分析</p>", unsafe_allow_html=True)
st.divider()
...
def get_stock_history(bs_code, start_date, end_date):
    try:
        ...
        df = df.rename(columns={
            ...
            'turn': '换手率', 'pctChg': '涨跌幅',
            'peTTM': 'PE_TTM', 'pbMRQ': 'PB_MRQ', 'psTTM': 'PS_TTM', 'pcfNcfTTM': 'PCF_TTM'
        })
...
with col1:
    ...
    <strong>获取字段:</strong><br>
    • 价格: 开/收/高/低/成交量/成交额<br>
    • 衍生: 振幅/涨跌额/换手率/涨跌幅<br>
    • 估值: PE_TTM / PB_MRQ / PS_TTM / PCF_TTM<br><br>
    <strong>导出文件:</strong><br>
    • stock_data.csv — 历史行情数据<br>
    • hs300_stock_list.csv — 成分股代码名称映射
    </div>
    """, unsafe_allow_html=True)
...
if extract_btn:
    ...
    if hs300_df is None:
        ...
        if all_data:
            ...
            with mc1:
                st.markdown(f"""
                <div class="metric-card"><h2 style="color:#1e40af; margin:0;">{len(combined_df):,}</h2>
                <p style="color:#64748b; margin:5px 0;">总数据行数</p></div>
                """, unsafe_allow_html=True)
            with mc2:
                st.markdown(f"""
                <div class="metric-card"><h2 style="color:#16a34a; margin:0;">{combined_df['股票代码'].nunique()}</h2>
                <p style="color:#64748b; margin:5px 0;">覆盖股票数</p></div>
                """, unsafe_allow_html=True)
            with mc3:
                st.markdown(f"""
                <div class="metric-card"><h2 style="color:#f59e0b; margin:0;">{len(all_data)}/{total}</h2>
                <p style="color:#64748b; margin:5px 0;">成功获取</p></div>
                """, unsafe_allow_html=True)
            with mc4:
                st.markdown(f"""
                <div class="metric-card"><h2 style="color:#dc2626; margin:0;">{len(failed_stocks)}</h2>
                <p style="color:#64748b; margin:5px 0;">失败数量</p></div>
                """, unsafe_allow_html=True)
            ...
            1. 点击上方按钮下载两个CSV文件到本地<br>
            2. 打开「沪深300股票预测分析」网页<br>
            3. 按顺序上传 <code>hs300_stock_list.csv</code> 和 <code>stock_data.csv</code><br>
            4. 点击「开始智能分析」获取预测结果
            </div>
            """, unsafe_allow_html=True)
        else:
